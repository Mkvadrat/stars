/*(function (w) {
    var q = [['setContext', 'TL-INT-hotelnord', 'ru']];
...
function bookingStepChanged(data) {
    ...
    if (window.yaCounter45482307) window.yaCounter45482307.hit('/' + data.step);
}

function bookingSuccess(data) {
    console.log('bookingSuccess');
    dataLayer.push({'event': 'travelline:bookingSuccess'});
    if (window.yaCounter45482307) window.yaCounter45482307.reachGoal('Bron_Travelline', {
...
function noAvailableRooms(data) {
    ...
    if (window.yaCounter45482307) window.yaCounter45482307.reachGoal('Booked_Up', {
        'now': data.now,
...
(function (w) {
    var q = [['setContext', 'TL-INT-hotelnord', 'ru'], ['embed', 'search-form', {container: 'tl-search-form'}]];
    var t = w.travelline = (w.travelline || {}), ti = t.integration = (t.integration || {});
    ti.__cq = ti.__cq ? ti.__cq.concat(q) : q;
    if (!ti.__loader) {
        ti.__loader = true;
...
(function (w) {
    ...
    var q = [['setContext', 'TL-INT-hotelnord', 'ru'], ['embed', 'booking-form', {
